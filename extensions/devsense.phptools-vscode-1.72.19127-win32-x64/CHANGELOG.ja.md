## 1.72.19127 (2026年7月28日)

### 修正

- Test executor によって実行される `--filter` オプションのバッククォートが安全に削除されます。 これにより、コマンドインジェクションの可能性が修正されます。 [#1061](https://github.com/DEVSENSE/phptools-docs/issues/1061)
- プレースホルダーを持つスニペットとしての括弧を含むコード補完を修正。 JSON RPC をデシリアライズする際の内部例外を修正。
- 複数行のBladeタグが存在する場合のBladeファイルでのリンク編集を修正しました。 [#2592](https://community.devsense.com/d/2592-issue-in-blade-directive)

### 改善

- 現在の位置で定義された変数は補完に表示されません。 [#1063](https://github.com/DEVSENSE/phptools-docs/issues/1063)
- 不明な型名の提案では、現在のコンテキストで意味を成さない型名は提案されません。
- コード補完における非推奨のシンボルは、デフォルトで取り消し線が引かれます（以前は通常の表示でした）。
- コード補完における関数パラメータはデフォルトで表示されます。
