...
- 通过测试执行器运行的 `--filter` 选项中的反引号被安全地移除。修复了可能的命令注入。[#1061](https://github.com/DEVSENSE/phptools-docs/issues/1061)
- 修复了带有占位符括号的代码自动完成。修复了解析 JSON RPC 时的内部异常。
- 修复了当 Blade 文件中存在多行 Blade 标签时的链接编辑。[#2592](https://community.devsense.com/d/2592-issue-in-blade-directive)

### 改进

- 自动完成不会显示在当前位置定义的变量。[#1063](https://github.com/DEVSENSE/phptools-docs/issues/1063)
- 对于未知类型名称的建议不会在当前上下文中建议不合理的类型名称。
- 代码自动完成中的弃用符号默认是删除线（以前显示为正常）。
- 代码自动完成中的函数参数默认会显示。

![代码建议列表](https://raw.githubusercontent.com/DEVSENSE/phptools-docs/refs/heads/master/docs/vscode/editor/imgs/completion-list.png)
